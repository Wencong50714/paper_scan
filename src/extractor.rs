...
impl ArchiveExtractor {
    ...
    fn scan_extracted_files(&self,
        ...
        if !extract_dir.exists() {
            return Err(anyhow::anyhow!("Extracted directory does not exist: {}", extract_dir.display()));
        }
        ...
        for entry in WalkDir::new(extract_dir) {
            ...
            if path.is_file() {
                match path.extension().and_then(|s| s.to_str()) {
                    ...
                    Some(ext) if ["png", "jpg", "jpeg", "pdf", "eps", "gif"].contains(&ext.to_lowercase().as_str()) => {
    ...
    fn find_main_tex_file(
        ...
        let main_names = ["main.tex", "paper.tex", "article.tex", "ms.tex", "template.tex"];
        for tex_file in tex_files {
            if let Some(file_name) = tex_file.file_name().and_then(|n| n.to_str()) {
                if main_names.iter().any(|name| file_name.eq_ignore_ascii_case(name)) {
                    return Ok(Some(tex_file.clone()));
                }
            }
        }
